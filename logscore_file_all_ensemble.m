...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function y = logscore_file_all_ensemble(ILIseasons,folder1,folder2,folder3,season,reg,ew)
name = [ folder1,'/',num2str(season),'-ew',num2str(ew),'_alltargets-reg',num2str(reg),'.csv'];
csvtable1 = readtable(name); 
name = [ folder2,'/',num2str(season),'-ew',num2str(ew),'_alltargets-reg',num2str(reg),'.csv'];
csvtable2 = readtable(name); 
name = [ folder3,'/',num2str(season),'-ew',num2str(ew),'_alltargets-reg',num2str(reg),'.csv'];
csvtable3 = readtable(name); 
csvtable = csvtable1;
csvtable(:,end) = array2table( 1/3 * (table2array(csvtable1(:,end)) + table2array(csvtable2(:,end)) + table2array(csvtable3(:,end))) );
%
...
for itar = 1:4
    ...
    low = max(1,find( [ 0:0.1:13 ,100] > gtr,1 ) - 6);
    up = max(1,find( [ 0:0.1:13 ,100] > gtr,1 ) + 4);
    zw3 = zw2(low:up,:);
